...
Northlight/Alan Wake 2, RE Engine). §3a–§3e are how a small, AI-driven team
...
- The foundation is **real audio assets**: a driving four-on-the-floor techno
  loop (~128–146 BPM, clear build/intensity) + **recorded SFX samples** (gunshot,
  reload, impact, hit, kill). The engine must support streaming/looping audio-file
  playback (WAV/OGG; MP3 via decode).
- Procedural/adaptive layers may sit *on top* (intensity layering with the run),
  but the base sounds are samples. Synthesised gunshots/music are a fallback, not
  the target.
...
**Next priorities** — bring the prototype from "works" to "premium," per §3:

1. **Real audio (§3d).** Swap procedural SFX/music for real files — a driving
   techno loop + recorded gunshot/reload/impact samples. This is the fastest
   single quality win and removes the synthesis dead-end.
2. **The "flatter" rendering pipeline (§3a), in order:** colour management + AgX
   tonemapping → PBR materials → shadows + baked lightmaps → SSAO → bloom/grade →
   TAA → volumetric fog. Verify each stage with the AI vision loop (§3c).
3. **Lock an art direction (§3b/§3e)** — palette, material language, fog/mood —
...
The honest test still stands (§7): the developer catches themselves replaying the
